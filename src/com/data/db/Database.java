...
/**
 * Class ini digunakan untuk menghubungkan dan memutuskan koneksi ke
 * <strong>Database</strong>, membackup <strong>Database</strong> dan menangani
 * masalah yang terjadi pada <strong>Database.</strong>
 * Kondisi seperti <strong>CRUD</strong> tidak tersedia di class ini. Kondisi
 * <strong>CRUD</strong> tersedia di class <code>Account</code> dan
 * <code>CovidCases</code>.
 * <BR><BR>
 * <strong>Database</strong> pada aplikasi ini sebagian besar menggunakan
 * <strong>MySQL Database</strong> dan sisanya menggunakan <strong>Database
 * Text</strong>.
 *
 *
...
public class Database {
     ...
     * <BR>
     * Pertama-tama method akan meregrister Driver yang dipakai. Driver yang
     * dipakai di aplikasi ini adalah <B>MySQL Driver</B>. Selanjutnya method
     * akan melakukan koneksi ke <B>Database</B>. Setelah berhasil terkoneksi ke
     * <B>Database</B> method akan membuat object <code>Statement</code>
     * <BR><BR>
     * Terdapat dua exception yang mungkin akan terjadi di method ini yaitu
     * <code>ClassNotFoundException</code> dan <code>SQLException</code>.
     * Exception akan ditangani dengan mendapatkan pesan error dari method
     * <code>getMessage</code> pada kedua exception tersebut. Beberapa pesan
     * error yang dapat ditanggani di method ini antara lain:
     * <UL>
     * <LI> <B>com.mysql.jdbc.Driver : </B> pesan error ini berarti aplikasi
     * tidak dapat menemukan Driver yang akan dipakai untuk menkoneksikan
     * aplikasi ke <B>Database</B>. Aplikasi akan secara otomatis keluar sendiri
     * jika mendapatkan pesan error ini.
     * <LI> <B>Communications link failure : </B> pesan error ini bearti MySQL
     * pada Komputer user belum diaktifkan. Aplikasi akan secara otomatis keluar
     * sendiri jika mendapatkan pesan error ini.
     * <LI> <B>Access denied for user 'root'@'localhost' (using password: YES) :
     * </B> pesan error ini bearti username atau password pada MySQL di komputer
     * user tidak cocok dengan username dan password yang ada di Aplikasi ini.
     * Aplikasi akan secara otomatis keluar sendiri jika mendapatkan pesan error
     * ini.
     * <LI> <B>Unknown database" : </B> pesan error ini bearti bahwa database
     * MySQL aplikasi ini tidak ada di komputer user. Method akan memulihkan
     * database secara otomatis dengan method <code>restoreDatabase()</code>
     * jika mendapatkan pesan error ini.
     * </UL>
     * <B>Note : </B> Jika pesan error tidak dikenali maka aplikasi akan keluar
     * sendiri.
     */
    ...
    /**
     * Digunakan untuk menutup koneksi dari <B>Database</B> MySQL. Koneksi dari
     * <B>Database</B> perlu ditutup jika sudah tidak digunakan lagi. Sebelum
     * menutup koneksi dari <B>Database</B> method akan mengecek apakah object
     * {@code Connection}, {@code Statement} dan {@code ResultSet} kosong atau
     * tidak. Jika tidak maka koneksi dari <B>Database</B> akan ditutup. Jika
     * tidak dicek kosong atau tidaknya object maka saat objek kosong lalu
     * dipaksa untuk menutup koneksi dari <B>Database</B>
     * maka akan menimbulkan exception {@code NullPointerException}.
     */
    public void closeConnection() {

        try {
            for (int i = 1; i <= this.jumlahKoneksi; i++) {
...
//            n.printStackTrace();
    ...
    /**
     * digunakan untuk mendapatkan fields yang berasal dari param field pada
     * method getData
     *
     * @param fields
     * @return
